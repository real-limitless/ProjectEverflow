"""Tests for the MySQL node executor (``n8n-nodes-base.mySql``).

Covers:

- ``mysql_response`` dict mock → envelope used per operation
- ``mysql_response`` callable mock receives
  ``(operation, query_or_table, params, item, ctx)``
- ``db_response`` fallback
- ``http_response`` fallback
- Offline execute: returns 2 rows
- Offline insert: affectedRows + insertId present
- Offline update: affectedRows=1, info present
- Offline upsert: affectedRows=2
- ``operation='execute'`` reflected on output
- ``query`` default from ``$json``
- ``table`` default from ``$json``
- Empty query → no item
- End-to-end: Manual Trigger → mySql (execute mock) → Set sees rows
...
from app.services.workflows.nodes.mysql import (
...
def _node(
    params: dict[str, Any],
    *,
    type_: str = "n8n-nodes-base.mySql",
    id_: str = "ms1",
    name: str = "MySQL",
...
# ── 1. mysql_response dict mock (execute) ───────────────────────────


@pytest.mark.asyncio
async def test_mysql_response_dict_mock_execute_used_verbatim() -> None:
    node = _node(
        {
            "operation": "execute",
    ...
    out = _out_items(
        await exec_mysql(node, [ExecutionItem(json={})], ctx=ctx)
    )
    assert len(out) == 2
    assert out[0].json["row"]["name"] == "Alice"
    assert out[0].json["rowCount"] == 2
    assert out[0].json["fieldCount"] == 2
    assert out[0].json["source"] == "mySql"
    assert "mockSource" not in out[0].json
    assert out[1].json["row"]["name"] == "Bob"


# ── 2. mysql_response callable mock signature ──────────────────────


@pytest.mark.asyncio
async def test_mysql_response_callable_mock_receives_args() -> None:
    ...
    ctx = _ctx({"mysql_response": _mock})
    item = ExecutionItem(json={"hint": 1})
    out = _out_items(await exec_mysql(node, [item], ctx=ctx))
...
async def test_offline_execute_returns_two_rows() -> None:
    ...
    out = _out_items(
        await exec_mysql(node, [ExecutionItem(json={})], ctx=_ctx())
    )
    assert len(out) == 2
    ...
    assert out[0].json["rowCount"] == 2
    assert out[0].json["fieldCount"] == 3
    assert out[0].json["source"] == "mySql"
...
# ── 6. Offline insert: affectedRows + insertId present ──────────────


@pytest.mark.asyncio
async def test_offline_insert_affected_rows_and_insert_id_present() -> None:
    node = _node(
        {
            "operation": "insert",
            "table": "users",
    ...
    assert isinstance(p["insertId"], int)
    assert p["fieldCount"] == 0
    assert "Records" in p["info"]
    assert p["source"] == "mySql"
    assert p["mockSource"] == "offline"


# ── 7. Offline update: affectedRows=1, info present ─────────────────


@pytest.mark.asyncio
async def test_offline_update_affected_rows_one_info_present() -> None:
    node = _node(
        {
            "operation": "update",
            "table": "users",
    ...
    assert p["insertId"] == 0
    assert p["fieldCount"] == 0
    assert "Rows matched" in p["info"]
    assert p["source"] == "mySql"
    assert p["mockSource"] == "offline"


# ── 8. Offline upsert: affectedRows=2 ───────────────────────────────


@pytest.mark.asyncio
async def test_offline_upsert_affected_rows_two() -> None:
    node = _node(
        {
            "operation": "upsert",
            "table": "users",
...
@pytest.mark.asyncio
async def test_operation_execute_reflected_in_output() -> None:
    node = _node(
        {
            "operation": "execute",
            "query": "SELECT 1",
...
async def test_query_default_from_json() -> None:
    ...
    out = _out_items(await exec_mysql(node, [item], ctx=_ctx()))
    assert len(out) == 2
    assert out[0].json["source"] == "mySql"
...
def test_default_operation_is_execute() -> None:
    assert MYSQL_DEFAULT_OPERATION == "execute"
    assert set(MYSQL_OPERATIONS) == {"execute", "insert", "update", "upsert"}
...
async def test_one_output_item_per_input_for_insert() -> None:
    ...
    out = _out_items(await exec_mysql(node, items, ctx=_ctx()))
    assert len(out) == 3
    for o in out:
        assert o.json["source"] == "mySql"
        assert o.json["affectedRows"] == 1
...
def test_descriptor_is_registered() -> None:
    ...
    assert "n8n-nodes-base.mySql" in REGISTRY
    assert "n8n-nodes-base.mySql" in SUPPORTED_NODE_TYPES
    assert SUPPORTED_NODE_TYPES["n8n-nodes-base.mySql"] == "output"
    desc = REGISTRY["n8n-nodes-base.mySql"]
    assert desc.executor.endswith(":exec_mysql")
    assert desc.category == "output"


# ── 18. End-to-end: Manual Trigger → mySql (execute mock) → Set ────


def _doc(nodes, connections):
    return {"name": "mysql-test", "nodes": nodes, "connections": connections}
...
@pytest.mark.asyncio
async def test_end_to_end_manual_mysql_execute_set_sees_rows() -> None:
    """Manual Trigger → mySql (execute, mysql_response mock) → Set pulls row fields."""
    mocks = {
        "mysql_response": {
            "rows": [
    ...
    doc = _doc(
        ...
        {
            "Start": {"main": [[{"node": "MySQL", "type": "main", "index": 0}]]},
            "MySQL": {"main": [[{"node": "Downstream", "type": "main", "index": 0}]]},
        },
    )
    ...
    ms_step = next(s for s in result.steps if s.node_name == "MySQL")
    assert ms_step.status == "success", ms_step.error
    assert ms_step.output_count == 2
