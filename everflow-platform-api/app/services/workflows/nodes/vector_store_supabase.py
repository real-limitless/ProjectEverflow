"""Supabase vector store executor for the LangChain ``vectorStoreSupabase`` node.
...
- ``id``            — auto-assigned integer id (1-based, per-store counter)
- ``content``       — the document body (``pageContent`` mirror)
- ``metadata``      — the document metadata dict
- ``embedding``     — ``list[float] | None`` — vector used for cosine
  similarity during ``retrieve``. Populated from
  ``ctx.mocks['embeddings_output']`` when a connected embedding model is
  present; otherwise ``None`` and the executor falls back to a keyword
  match.
...
# Module-level per-run Supabase store. Key: f"{run_id}:{node.id}:{tableName}".
_SUPABASE_STORES: dict[str, list[dict[str, Any]]] = {}
...
def _coerce_doc(raw: Any) -> dict[str, Any] | None:
    ...
    for inputs that are clearly not document-shaped (e.g. ``None``).
    """
    if raw is None:
        return None
    ...
    if isinstance(raw, str):
        return {"pageContent": raw, "metadata": {}, "embedding": None}
    return {"pageContent": str(raw), "metadata": {}, "embedding": None}
...
def _resolve_vector_store_mock(
    ...
    result = raw(mode, items, params, ctx) if callable(raw) else raw
...
def _resolve_query(
    ...
    Order (mirrors ``chainRetrievalQa``):
...
async def exec_vector_store_supabase(
    ...
    *,
    ...
    """Supabase vector store — insert / load / retrieve with cosine similarity.
    ...
    ``f"{run_id}:{node.id}:{tableName}"`` (default ``tableName="documents"``)
    so multiple runs, multiple vector-store nodes, and multiple logical
    tables in the same run stay isolated. The ``queryName`` parameter
    (default ``"match_documents"``) is echoed into the output payload the
    way the real Supabase RPC name would be.
    ...
    ``id``, its ``pageContent``, ``metadata``, and (when available)
    ...
    ``{document, score, tableName, queryName, source}``.
    """
    params = node.parameters or {}
    ...
    query_name = (
        str(params.get("queryName") or "match_documents").strip() or "match_documents"
    )
    ...
    if mode in ("insert", "load"):
        ...
        bucket = _SUPABASE_STORES.setdefault(key, [])
        ...
        for doc in docs:
            ...
            if doc.get("embedding") is not None:
                entry["embedding"] = list(doc["embedding"])
            bucket.append(entry)
            next_id += 1
    ...
    # insert node's bucket key. When the mock is absent, the executor
    # falls back to the run-scoped Supabase store populated by an earlier
    # insert / load call on the same ``(run_id, node_id, table_name)`` key.
    mock_docs = _resolve_vector_store_mock(ctx, mode, items, params)
    if mock_docs is not None:
        bucket = [
            {
