"""Qdrant vector store executor for the LangChain ``vectorStoreQdrant`` node.
...
Stored records are Qdrant-flavoured dicts:

- ``id``        — auto-assigned string id (``"doc-1"``, ``"doc-2"`` …)
- ``vector``    — ``list[float] | None`` — vector for cosine similarity
  during ``retrieve``. Populated from ``ctx.mocks['embeddings_output']``
  when a connected embedding model is present; otherwise ``[]`` and the
  executor falls back to a keyword match against ``payload['text']``.
- ``payload``   — the document metadata dict (which always carries the
  document text under the ``"text"`` key so the pageContent survives a
  round-trip in payload form).
...
# Module-level per-run Qdrant-style collection.
# Key: f"{run_id}:{node.id}:{collectionName}".
_QDRANT_COLLECTIONS: dict[str, list[dict[str, Any]]] = {}
...
def _store_key(
    ...
    return f"{ctx.run_id or 'no-run'}:{node.id}:{collection_name}"
...
def _coerce_doc(raw: Any) -> dict[str, Any] | None:
    ...
    so the retrieve path can use a pre-supplied ``score`` from the mock.
    """
    if raw is None:
        return None
    if isinstance(raw, dict):
        ...
        embedding = raw.get("embedding") or raw.get("vector")
...
def _coerce_collection_name(value: Any) -> str:
    """Best-effort coercion of a ``collectionName`` parameter to a non-empty string."""
    if value is None:
        return "n8n-qdrant-collection"
    if isinstance(value, str):
        s = value.strip()
        return s or "n8n-qdrant-collection"
    s = str(value).strip()
    return s or "n8n-qdrant-collection"


def _coerce_url(value: Any) -> str:
    """Best-effort coercion of a ``url`` parameter; default ``http://localhost:6333``."""
    if value is None:
        return "http://localhost:6333"
    if isinstance(value, str):
        s = value.strip()
        return s or "http://localhost:6333"
    s = str(value).strip()
    return s or "http://localhost:6333"
...
async def exec_vector_store_qdrant(
    ...
    *,
    ...
    """Qdrant vector store — insert / load / retrieve with cosine similarity.
    ...
    ``f"{run_id}:{node.id}:{collectionName}"`` (default
    ``collectionName="n8n-qdrant-collection"``) so multiple runs, multiple
    vector-store nodes, and multiple collections in the same run stay
    isolated. The ``url`` parameter (default
    ``"http://localhost:6333"``) is echoed into the output payload the
    way the real Qdrant HTTP endpoint would be; **no live call is made**.
    ...
    ``id`` (``"doc-1"`` …), its ``vector`` (embedding vector or empty
    list), and ``payload`` (which carries the document text under the
    ...
    against the payload text (when no vectors are present), the
    top-``topK`` docs are returned, each carrying
    ``{document, score, id, collectionName, url, source}``.
    """
    params = node.parameters or {}
    ...
    url = _coerce_url(params.get("url"))

    key = _store_key(ctx, node, collection_name)
    ...
    if mode in ("insert", "load"):
        ...
        for doc in docs:
            ...
            # Qdrant payload is plain dicts; the text rides along so
            # the pageContent survives a round-trip in payload form.
    ...
    bucket = list(_QDRANT_COLLECTIONS.get(key) or [])
    ...
    if mock_docs is not None:
        bucket = [
            {
                "id": f"doc-{i + 1}",
                "vector": d.get("embedding") or d.get("vector"),
                "payload": {
    ...
    for item in items:
        ...
        if bucket and any(
            d.get("vector") if isinstance(d, dict) else False for d in bucket
        ) and model_name:
        ...
        for entry in bucket:
            ...
            if isinstance(pre_score, (int, float)) and not isinstance(pre_score, bool):
                ...
                continue
            doc_vec = entry.get("vector") if isinstance(entry, dict) else None
            payload = entry.get("payload") or {} if isinstance(entry, dict) else {}
            text = str(payload.get("text", "")) if isinstance(payload, dict) else ""
            if (
                query_embedding is not None
                and isinstance(doc_vec, list)
                and doc_vec
            ):
                score = _cosine_similarity(query_embedding, [float(v) for v in doc_vec])
            else:
